...
\begin{titlepage}
    ...
    {\huge \bfseries \color{navyblue} EXAMEN FINAL — PARTE I \par}
    \vspace{0.4cm}
...
\paragraph*{Cambio 1: Commit \texttt{971cd4a} (Despliegue de la Estrategia Canary)}
\begin{itemize}[leftmargin=1.5em]
    \item \textbf{Timestamp Git (\texttt{git log}):} Sábado 25/Jul/2026 a las \textbf{13:36:56}.
    \item \textbf{Timestamp CI/CD (GitHub Actions):} Finalizado en \textbf{1 min 31 s} a las \textbf{13:38:27} (ver figura \ref{fig:deployment_frequency}).
    \item \textbf{Rollout Kubernetes (\texttt{kubectl rollout status}):} Estado verificado en \textbf{0.08 s} (79 ms).
    \item \textbf{Cálculo exacto del Lead Time:} 13h 38m 27.08s $-$ 13h 36m 56s = \textbf{1 minuto y 31.08 segundos}.
\end{itemize}

\paragraph*{Cambio 2: Commit \texttt{58e2d54} (Corrección Final de Código)}
\begin{itemize}[leftmargin=1.5em]
    \item \textbf{Timestamp Git (\texttt{git log}):} Sábado 25/Jul/2026 a las \textbf{21:15:09} (ver figura \ref{fig:git_log_commit}).
    \item \textbf{Timestamp CI/CD (GitHub Actions):} Finalizado en \textbf{1 min 31 s} a las \textbf{21:16:40} (ver figura \ref{fig:deployment_frequency}).
    \item \textbf{Rollout Kubernetes (\texttt{kubectl rollout status}):} Estado verificado en \textbf{0.08 s} (79 ms, ver figura \ref{fig:kubectl_rollout_measure}).
...
A partir de estas marcas de tiempo, se valida que el equipo realizó 6 integraciones y entregas exitosas a producción en un período de 24 horas, alcanzando una frecuencia real de \textbf{6 despliegues/día}.
...
\section{Componentes Adicionales de Buenas Prácticas}

De acuerdo con la rúbrica del examen, se exigía implementar al menos 2 de 3 componentes adicionales de buenas prácticas. Nuestro equipo implementó **los 3 componentes completos**, garantizando los **+2 puntos extra de bonificación**:

\subsection{Componente 1: Manejo de Secretos (\texttt{API\_KEY})}
Se creó un objeto \texttt{Secret} nativo de Kubernetes (\texttt{k8s/secret.yaml}) con la clave ficticia \texttt{API\_KEY: "my-super-secret-api-key-12345"}. Esta credencial se consume en el contenedor vía variable de entorno utilizando \texttt{secretKeyRef} en \texttt{k8s/deployment.yaml}, garantizando que ninguna contraseña en texto plano quede guardada en el código ni versionada en Git. *(Ver Evidencia en la Figura \ref{fig:evidencia2_secretos})*.

\subsection{Componente 2: Escaneo de Seguridad en CI con Trivy}
Se integró el análisis de seguridad con la acción \texttt{aquasecurity/trivy-action} en el pipeline de GitHub Actions (\texttt{.github/workflows/ci-cd.yml}). El paso está configurado con \texttt{exit-code: 1} y \texttt{severity: CRITICAL}, aplicando el principio \textit{Fail-Fast}: si se detecta cualquier vulnerabilidad crítica en la imagen Docker, la ejecución se cancela antes de la publicación en GHCR. *(Ver Figuras \ref{fig:trivy_error} y \ref{fig:evidencia3_trivy})*.

\subsection{Componente 3: Readiness Probe Realista con Arranque Lento}
Se configuró la variable de entorno \texttt{STARTUP\_DELAY\_SECONDS: 10} en la aplicación para simular un tiempo de arranque de conexión a base de datos. Se ajustó el \texttt{readinessProbe} con \texttt{initialDelaySeconds: 12} y \texttt{periodSeconds: 5}. Esto garantiza que Kubernetes no envíe tráfico al Pod ni lo elimine durante sus primeros 10 segundos de inicio, esperando a que el endpoint \texttt{/health} responda \texttt{200 OK}. *(Ver Evidencia en la Figura \ref{fig:evidencia4_readiness})*.
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Solución:} Se unificaron los nombres de las imágenes en todos los manifiestos YAML apuntando al usuario propietario del repositorio (\texttt{ghcr.io/\allowbreak xavierstalin/\allowbreak inventario-app:latest}) y se configuró la visibilidad del paquete como \textbf{Pública} en GitHub Packages.
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Solución:} Se ejecutó una prueba desde dentro del clúster mediante un Pod de prueba efímero (\texttt{kubectl run test-canary}). Al consultar directamente la dirección DNS interna del servicio, \texttt{Kube-Proxy} realizó el balanceo real repartiendo ~80\% a la versión \texttt{v1} y ~20\% a la versión \texttt{v2}. *(Ver Figura \ref{fig:evidencia1_canary})*.
\end{itemize}
